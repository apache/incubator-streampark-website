...
当需要**升级现有安装**时，请遵循以下指南：

-   确保按照版本号顺序逐个执行 `upgrade` 目录下的脚本。
-   特别注意，如果需要跨多个版本升级，必须一步步执行每个版本的升级脚本。

例如，从版本 2.0.0 升级到 2.0.2，应该这样操作：

1. 首先执行 2.0.0 至 2.0.1 的升级脚本。
2. 然后执行 2.0.1 至 2.0.2 的升级脚本。
